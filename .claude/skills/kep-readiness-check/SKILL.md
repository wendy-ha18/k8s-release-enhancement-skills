...
(relative to the current working directory — the repo root, alongside
`README.md`, per this skill's usage instructions; the `report/` directory
is created if it doesn't exist yet) and never overwrites a previous run's
...
- PyYAML (`python3 -c "import yaml"` to check; `pip3 install pyyaml` if
  missing).
- Write access to `report/` under the current working directory (expected
  to be the repo root, alongside `README.md`) — the script creates the
  directory automatically if it doesn't exist yet and writes the report
  file there.
